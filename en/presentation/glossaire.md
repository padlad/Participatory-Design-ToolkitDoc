...
* Visualization (**fr.** *Visualisation*) - Visualizations are spatial representation of data in visual (textual or graphical) form. They are designed to transform data into actionable insight which can be easily perceived and interpreted.
* Panel (**fr.** *Panneau*) - Depending on its type and nature, an indicator can be associated with several types of visualizations. Nevertheless, for a given situation and a specific analytical need, not all possible representations are necessarily relevant. Indeed, a limited set of possibilities would allow to better identify the properties targeted by the analyst. 
